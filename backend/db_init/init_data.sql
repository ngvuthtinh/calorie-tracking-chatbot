...
CREATE TABLE exercise_entry (
    ...
    burned_kcal DECIMAL(10, 2) DEFAULT 0,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    is_deleted BOOLEAN DEFAULT FALSE,
...
FROM food_catalog
WHERE
    name_normalized = 'com_tam';

INSERT INTO
    food_alias (alias_normalized, food_id)
SELECT 'bún bò huế', id
FROM food_catalog
WHERE
    name_normalized = 'bun_bo_hue';
...
FROM food_catalog
WHERE
    name_normalized = 'hu_tieu';

INSERT INTO
    food_alias (alias_normalized, food_id)
SELECT 'coconut water', id
FROM food_catalog
WHERE
    name_normalized = 'coconut_water';
...
-- Get the last ID (which should be 1 since we just inserted)
